...
de.philippsoehnein.teaTimer.vendor.aes = {
  Cipher : function(input, w) {    // main Cipher function [¤5.1]
    ...
    state = de.philippsoehnein.teaTimer.vendor.aes.AddRoundKey(state, w, 0, Nb);
  
    for (var round=1; round<Nr; round++) {
      state = de.philippsoehnein.teaTimer.vendor.aes.SubBytes(state, Nb);
      state = de.philippsoehnein.teaTimer.vendor.aes.ShiftRows(state, Nb);
      state = de.philippsoehnein.teaTimer.vendor.aes.MixColumns(state, Nb);
      state = de.philippsoehnein.teaTimer.vendor.aes.AddRoundKey(state, w, round, Nb);
    }
  
    state = de.philippsoehnein.teaTimer.vendor.aes.SubBytes(state, Nb);
    state = de.philippsoehnein.teaTimer.vendor.aes.ShiftRows(state, Nb);
    state = de.philippsoehnein.teaTimer.vendor.aes.AddRoundKey(state, w, Nr, Nb);
  ...
  SubBytes : function(s, Nb) {    // apply SBox to state S [¤5.1.1]
    for (var r=0; r<4; r++) {
      for (var c=0; c<Nb; c++) s[r][c] = de.philippsoehnein.teaTimer.vendor.aes.Sbox[s[r][c]];
    }
    return s;
  },
  ...
  KeyExpansion : function(key) {  // generate Key Schedule (byte-array Nr+1 x Nb) from Key [¤5.2]
    ...
    for (var i=Nk; i<(Nb*(Nr+1)); i++) {
      ...
      if (i % Nk == 0) {
        temp = de.philippsoehnein.teaTimer.vendor.aes.SubWord(de.philippsoehnein.teaTimer.vendor.aes.RotWord(temp));
        for (var t=0; t<4; t++) temp[t] ^= de.philippsoehnein.teaTimer.vendor.aes.Rcon[i/Nk][t];
      } else if (Nk > 6 && i%Nk == 4) {
        temp = de.philippsoehnein.teaTimer.vendor.aes.SubWord(temp);
      }
  ...
  SubWord : function(w) {    // apply SBox to 4-byte word w
    for (var i=0; i<4; i++) w[i] = de.philippsoehnein.teaTimer.vendor.aes.Sbox[w[i]];
    return w;
  },
  ...
  AESEncryptCtr : function(plaintext, password, nBits) {
    ...
    var key = de.philippsoehnein.teaTimer.vendor.aes.Cipher(pwBytes, de.philippsoehnein.teaTimer.vendor.aes.KeyExpansion(pwBytes));  // gives us 16-byte key
    ...
    var keySchedule = de.philippsoehnein.teaTimer.vendor.aes.KeyExpansion(key);
    ...
    for (var b=0; b<blockCount; b++) {
      ...
      var cipherCntr = de.philippsoehnein.teaTimer.vendor.aes.Cipher(counterBlock, keySchedule);  // -- encrypt counter block --
  ...
  AESDecryptCtr : function(ciphertext, password, nBits) {
    ...
    var key = de.philippsoehnein.teaTimer.vendor.aes.Cipher(pwBytes, de.philippsoehnein.teaTimer.vendor.aes.KeyExpansion(pwBytes));
    key = key.concat(key.slice(0, nBytes-16));  // expand key to 16/24/32 bytes long
  ...
  encodeBase64 : function(utf8encode) {  // http://tools.ietf.org/html/rfc4648
    ...
    for (c=0; c<plain.length; c+=3) {  // pack three octets into four hexets
      ...
      var b64=de.philippsoehnein.teaTimer.vendor.aes.b64;
   ...
   */ 
  decodeBase64 : function(utf8decode) {
...
String.prototype.encodeBase64 = de.philippsoehnein.teaTimer.vendor.aes.encodeBase64;
String.prototype.decodeBase64 = de.philippsoehnein.teaTimer.vendor.aes.decodeBase64;
String.prototype.encodeUTF8   = de.philippsoehnein.teaTimer.vendor.aes.encodeUTF8;
String.prototype.decodeUTF8   = de.philippsoehnein.teaTimer.vendor.aes.decodeUTF8;